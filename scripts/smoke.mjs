...
const run = async () => {
  ...
  // ------------------------------------------------------------------
  // ------------------------------------------------------------------
  console.log("\n[15] Retired internal question platform: routes gone, no dead links, external entry hidden until configured");
  const gone = ["/exams", "/exams/electrostatics-check", "/results", "/admin/assessment"];
  for (const path of gone) {
    const r = await student.get(path);
    check(`retired route ${path} no longer renders (not 200)`, r.status !== 200, `got ${r.status}`);
  }
  // External Questions Platform entry stays hidden while unconfigured/disabled.
  const dashNoEntry = await student.get("/dashboard");
  check("question-platform entry hidden when no URL is configured", dashNoEntry.status === 200 && !dashNoEntry.text.includes("question-platform-card"), `got ${dashNoEntry.status}`);
  ...
  // The internal assessment hub was retired (external Questions Platform).
  const assessHubGone = await admin.get("/admin/assessment");
  check("retired /admin/assessment hub is gone", assessHubGone.status !== 200, `got ${assessHubGone.status}`);
