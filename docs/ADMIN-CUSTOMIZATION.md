...
- المصدر: حساب يُنشأ عبر `scripts/bootstrap-admin.mjs`.
- البريد: متغير البيئة `ADMIN_BOOTSTRAP_EMAIL` أو `.dev.vars` أو `--email=you@example.com`.
- مثال التطوير في `.dev.vars.example`: `admin@educore.local` (اسم تقني داخلي — لا يظهر للزوّار).
- كلمة المرور: تُمرَّر عبر البيئة عند الإنشاء ولا تُطبع في السجلات. غيّرها فورًا من **الملف الشخصي ← الأمان**.
- تغيير البريد لاحقًا يتم من إدارة المستخدمين (حسابك) أو بإنشاء مشرف جديد ثم تعطيل القديم. لا تشارك أسرار الدخول.
