...
> Gateway adapters (Paymob / Fawry / Stripe) ship only in Phase 5 **after** a verification ADR
...
### Gateways (Phase 5+, verification-gated)
...
`payments` (full state trail) · `payment_events` (webhook inbox: raw sanitized payload, signature validity, idempotent processing by `provider_event_id`) · `refunds` · daily reconciliation job (Phase 5): gateway `fetchRemoteStatus` sweep for `pending` payments older than TTL → auto-expire; mismatches flagged for admin (`under_review`). No card numbers/CVV ever stored — gateway-only.
...
## 6. Verification log (fill at Phase 5, one row per provider)
