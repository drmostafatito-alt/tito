...
export default function AdminCmsMenus({ loaderData }: Route.ComponentProps) {
  ...
  const renderItem = (item: { id: string; labelAr: string; labelEn: string; href: string; external: boolean; icon: string | null; visible: boolean }, depth: number) => (
    <li key={item.id} className={`flex flex-wrap items-center gap-2 rounded-lg border border-line px-3 py-2 ${item.visible ? "bg-white" : "bg-slate-50 opacity-60"} ${depth > 0 ? "ms-6" : ""}`}>
      ...
      <details className="w-full">
        ...
        <Form method="post" className="mt-2 grid gap-3 sm:grid-cols-2">
          ...
          <div className="flex flex-col">
            <span className="mb-1 text-sm font-medium text-ink">{L("cms.f.icon")}</span>
            <select name="icon" defaultValue={item.icon ?? ""} className="h-[42px] rounded-lg border border-line bg-white px-3 text-sm">
              <option value="">—</option>
      ...
      <nav className="flex flex-wrap gap-2" aria-label={L("cms.ui.menus")}>
        {LOCATIONS.map((loc) => (
          <Link
            ...
            className={`inline-flex min-h-11 items-center rounded-lg px-4 text-sm font-medium ${location === loc ? "bg-brand-700 text-white" : "border border-line bg-white text-ink hover:bg-slate-50"}`}
          >
            {L(`cms.ui.menu.${loc}`)}
          </Link>
        ))}
      ...
      <Card>
        ...
        <CardBody>
          <Form method="post" className="grid gap-3 sm:grid-cols-2">
            ...
            <div className="flex flex-col">
              <span className="mb-1 text-sm font-medium text-ink">{L("cms.f.icon")}</span>
              <select name="icon" defaultValue="" className="h-[42px] rounded-lg border border-line bg-white px-3 text-sm">
                <option value="">—</option>
                {ICON_IDS.map((id) => <option key={id} value={id}>{id}</option>)}
              </select>
            </div>
            <div className="flex flex-col">
              <span className="mb-1 text-sm font-medium text-ink">{L("cms.ui.parent")}</span>
              <select name="parentId" defaultValue="" className="h-[42px] rounded-lg border border-line bg-white px-3 text-sm">
