  ...
  return (
    <div className="space-y-4" key={`order-${order?.id}`}>
      <nav className="text-xs text-ink-muted">
        <Link to="/admin/commerce?tab=orders" className="hover:text-brand-700">{t(locale, "commerceAdmin.title")}</Link>
        ...
        <CardBody className="space-y-4">
          {payments.map((p) => (
            <div key={p.id} className="space-y-2 rounded-xl border border-line p-3" data-testid="admin-payment-card">
              ...
              {p.evidence && (
                <div className="rounded-lg bg-sand-100 p-3 text-xs" data-testid="admin-evidence">
                  <p className="font-semibold text-ink-soft">{t(locale, "commerceAdmin.evidence")}</p>
                  ...
                  {p.proofPreview ? (
                    <figure className="mt-2">
                      <img src={p.proofPreview.url} alt={t(locale, "commerce.proofImageAlt")} className="max-h-64 rounded-lg border border-line bg-surface object-contain" data-testid="admin-proof-img" />
              ...
              {p.rejection && (
                <p className="text-xs text-error">{t(locale, "commerceAdmin.rejectedNote")}: {p.rejection.reason}</p>
              )}
              {p.refund && (
                <p className="text-xs text-error">{t(locale, "commerceAdmin.refundNote")}: {p.refund.reason}</p>
              )}
              {p.status === "under_review" && perms.payments && (
                <div className="flex flex-wrap items-end gap-3 bg-sand-100 p-3">
              ...
              {p.status === "paid" && perms.refunds && (
                <Form method="post" className="flex flex-wrap items-end gap-2 bg-error-soft p-3">
