...
function Pager({ page, total, tab, extra }: { page: number; total: number; tab: string; extra: Record<string, string> }) {
  ...
  return (
    <div className="flex items-center justify-between pt-2 text-sm">
      {page > 1 ? <Link className="text-ink hover:underline" to={withPage(page - 1)} data-testid="sec-prev">{t(locale, "securityAdmin.prevPage")}</Link> : <span />}
      <span className="text-xs text-ink-muted">{t(locale, "securityAdmin.pageOf", { page, total: totalPages })}</span>
      {page < totalPages ? <Link className="text-ink hover:underline" to={withPage(page + 1)} data-testid="sec-next">{t(locale, "securityAdmin.nextPage")}</Link> : <span />}
    </div>
  );
}
      ...
      <div className="flex gap-2" data-testid="sec-tabs">
        {(["events", "sessions"] as const).map((tb) => (
          <Link
            ...
            className={`inline-flex min-h-9 items-center rounded-lg px-3 py-1.5 text-sm font-medium ${tab === tb ? "bg-slate-900 text-white" : "bg-white text-ink-muted ring-1 ring-line hover:bg-slate-50"}`}
          >
          ...
          <CardBody className="space-y-3">
            ...
            {loaderData.eventsQ.rows.map((e) => (
              <div key={e.id} className="flex flex-wrap items-center justify-between gap-2 border-b border-line py-2 text-sm last:border-0" data-testid="security-event-row">
                <span className="text-xs font-medium text-ink" title={e.type}>
                  {t(locale, `securityAdmin.ev_${e.type}`)}
                </span>
                {e.userEmail ? (
                  <Link to={`/admin/users/${e.userId}`} className="text-xs text-ink hover:underline" dir="ltr">{e.userEmail}</Link>
                ) : (
            ...
            {loaderData.sessionsQ.rows.map((s) => (
              <div key={s.id} className="flex flex-wrap items-center justify-between gap-2 border-b border-line py-2 text-sm last:border-0" data-testid="session-row">
                <div className="flex min-w-0 flex-col">
                  <Link to={`/admin/users/${s.userId}`} className="truncate text-xs text-ink hover:underline" dir="ltr">{s.userEmail}</Link>
