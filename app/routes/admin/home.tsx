...
function StatCard({ label, value, testid, mono }: { label: string; value: string | number; testid: string; mono?: boolean }) {
  return (
    <Card className="border-t-2 border-t-brand-800">
      <CardBody>
        <p className={`text-2xl font-bold tabular-nums text-ink ${mono ? "font-mono text-lg" : ""}`} dir="ltr" data-testid={testid}>
...
function Section({ title, cols, children }: { title: string; cols?: string; children: ReactNode }) {
  return (
    <section className="flex flex-col gap-3">
      <h2 className="flex items-center gap-2 text-sm font-bold uppercase tracking-wide text-ink"><span className="h-3 w-3 shrink-0 bg-accent-500" aria-hidden="true" />{title}</h2>
...
const QUICK = [
  { to: "/admin/assessment/questions/new", key: "dash.qQuestion", tone: "bg-brand-800 text-white" },
  { to: "/admin/assessment/exams/new", key: "dash.qExam", tone: "bg-brand-800 text-white" },
  { to: "/admin/files", key: "dash.qMedia", tone: "bg-brand-800 text-white" },
  { to: "/admin/content", key: "dash.qContent", tone: "bg-brand-800 text-white" },
  { to: "/admin/announcements?new=1", key: "dash.qAnnounce", tone: "bg-brand-800 text-white" },
] as const;
...
export default function AdminHome({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="flex flex-col gap-6">
      {/* Welcome */}
      <div className="flex flex-wrap items-end justify-between gap-4 border-b-2 border-brand-800 pb-4">
        <div>
          <h1 className="sig-display flex items-center gap-2.5 text-3xl text-ink">
            <span className="h-4 w-4 shrink-0 bg-accent-500" aria-hidden="true" />
            {t(locale, "dash.welcome", { name: loaderData.adminName || "Admin" })}
          </h1>
          <p className="mt-1.5 text-sm text-ink-muted">{t(locale, "dash.subtitle")}</p>
        </div>
      ...
      <div className="flex flex-col gap-2">
        <h2 className="flex items-center gap-2 text-sm font-bold uppercase tracking-wide text-ink"><span className="h-3 w-3 shrink-0 bg-accent-500" aria-hidden="true" />{t(locale, "dash.quickTitle")}</h2>
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5" data-testid="quick-actions">
          {QUICK.map((qa) => (
            <Link
              key={qa.to}
              to={qa.to}
              className={`inline-flex min-h-12 items-center justify-center gap-2 rounded-[var(--radius-btn)] px-3 py-2.5 text-sm font-semibold text-white transition hover:bg-brand-900 ${qa.tone}`}
            >
              {t(locale, qa.key)}
            </Link>
          ))}
      ...
      {o ? (
        <>
          ...
          <Section title={t(locale, "admin.secOps")}>
            <Link to="/admin/content" className="flex flex-col items-start justify-between gap-1 rounded-[var(--radius-card)] border border-line border-t-2 border-t-brand-800 bg-white p-3 text-start transition hover:border-b-brand-800 hover:bg-slate-50">
              <span className="text-sm font-medium text-ink-muted">{t(locale, "admin.mPublishedCourses")}</span>
              <span className="text-2xl font-bold tabular-nums text-ink" dir="ltr" data-testid="home-metric-published-courses">{loaderData.ops!.publishedCourses}</span>
            </Link>
            <Link to="/admin/assignments" className="flex flex-col items-start justify-between gap-1 rounded-[var(--radius-card)] border border-line border-t-2 border-t-brand-800 bg-white p-3 text-start transition hover:border-b-brand-800 hover:bg-slate-50">
              <span className="text-sm font-medium text-ink-muted">{t(locale, "admin.mPendingAssignGrading")}</span>
              <span className="text-2xl font-bold tabular-nums text-ink" dir="ltr" data-testid="home-metric-pending-assignments">{loaderData.ops!.pendingAssignmentGrading}</span>
            </Link>
            <Link to="/admin/assessment" className="flex flex-col items-start justify-between gap-1 rounded-[var(--radius-card)] border border-line border-t-2 border-t-brand-800 bg-white p-3 text-start transition hover:border-b-brand-800 hover:bg-slate-50">
              <span className="text-sm font-medium text-ink-muted">{t(locale, "admin.mPendingEssayGrading")}</span>
              <span className="text-2xl font-bold tabular-nums text-ink" dir="ltr" data-testid="home-metric-pending-essays">{loaderData.ops!.pendingEssayGrading}</span>
            </Link>
          </Section>
        </>
      ) : (
      ...
      <Card data-testid="course-performance">
        <CardHeader
          ...
          action={<Link to="/admin/content" className="sig-u text-sm font-bold text-ink">{t(locale, "dash.viewAllCourses")}</Link>}
        />
        <CardBody>
          {loaderData.courses.length === 0 ? (
            <EmptyState
              ...
              action={<Link to="/admin/content" className="inline-flex min-h-9 items-center rounded-[var(--radius-btn)] bg-brand-700 px-3 text-sm font-semibold text-white hover:bg-brand-800">{t(locale, "dash.courseEmptyAction")}</Link>}
            />
          ) : (
            <ul className="flex flex-col gap-2">
              {loaderData.courses.map((c) => (
                <li key={c.id}>
                  <Link to={`/admin/content/course/${c.id}`} className="flex flex-wrap items-center gap-3 rounded-lg border border-line px-3 py-2.5 transition hover:border-ink">
                    {c.thumbnailFileId ? (
                      <img src={`/files/${c.thumbnailFileId}`} alt="" className="h-12 w-16 rounded-md border border-line bg-slate-50 object-cover" />
                    ) : (
                      <span className="flex h-12 w-16 items-center justify-center rounded-md bg-slate-100 text-xl text-ink-muted" aria-hidden="true">○</span>
                    )}
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-sm font-bold text-ink">{courseLabel(c, locale)}</p>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          <CardHeader title={t(locale, "admin.recentActivity")} action={<Link to="/admin/analytics" className="sig-u text-sm font-bold text-ink">{t(locale, "admin.navAnalytics")}</Link>} />
          <CardBody>
        ...
        <Card>
          <CardHeader title={t(locale, "admin.mRecentRegs")} action={<Link to="/admin/users" className="sig-u text-sm font-bold text-ink">{t(locale, "admin.navUsers")}</Link>} />
          <CardBody>
              ...
              <ul className="flex flex-col gap-2.5">
                {o.users.recent.map((u) => (
                  <li key={u.id} className="flex items-center justify-between gap-2 text-sm" data-testid="recent-reg-row">
                    <Link to={`/admin/users/${u.id}`} className="sig-u truncate font-semibold text-ink">{u.fullName}</Link>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card>
          <CardHeader title={t(locale, "admin.recentAudit")} action={<Link to="/admin/audit" className="sig-u text-sm font-bold text-ink">{t(locale, "admin.navAudit")}</Link>} />
          <CardBody>
