...
function Brand({ appName, locale, onLight = false }: { appName: string; locale: Locale; onLight?: boolean }) {
  return (
    <Link to="/admin" aria-label={appName} className="flex items-center gap-2.5">
      <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-[var(--radius-btn)] bg-accent-500 text-white" aria-hidden="true">
        <AdminIcon name="logo" className="h-5 w-5" />
      </span>
      <span className="min-w-0">
        <span className={`block truncate text-sm font-bold leading-tight ${onLight ? "text-ink" : "text-white"}`}>{appName}</span>
        <span className={`block text-[11px] font-semibold ${onLight ? "text-ink-muted" : "text-white/60"}`}>{t(locale, "nav.adminLabel")}</span>
      </span>
    </Link>
  );
}
...
function PageCrumbs({ pathname, locale }: { pathname: string; locale: Locale }) {
  ...
  if (pathname === "/admin" || pathname === "/admin/") {
    return <span className="truncate text-sm font-bold text-ink">{root}</span>;
  }
  const match = resolveNavItem(pathname);
  const label = match ? t(locale, match.item.labelKey) : root;
  return (
    <nav aria-label="Breadcrumb" className="flex min-w-0 items-center gap-1.5 text-sm">
      <Link to="/admin" className="truncate font-semibold text-ink-muted hover:text-ink">
        {root}
      </Link>
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        className="flex h-10 w-10 items-center justify-center rounded-[var(--radius-btn)] bg-brand-800 text-sm font-bold text-white transition-colors hover:bg-brand-900"
      >
        {initial}
      </button>
      {open && (
        <div
          role="menu"
          className="absolute end-0 top-12 z-50 w-60 overflow-hidden rounded-[var(--radius-card)] border border-line bg-white shadow-lg"
        >
          <div className="border-b border-line px-4 py-3">
            <p className="truncate text-sm font-bold text-ink" dir="auto">
              {fullName || "Admin"}
            </p>
            <p className="truncate text-xs tabular-nums text-ink-muted" dir="ltr">
              {email}
            </p>
          </div>
          <div className="p-1.5">
            <Link
              ...
              role="menuitem"
              className="flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium text-ink hover:bg-slate-100"
            >
...
export default function AdminLayout({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="flex min-h-dvh bg-white">
      {/* Desktop sidebar */}
      <aside
        className={`sticky top-0 hidden h-dvh shrink-0 flex-col border-e border-brand-800 bg-brand-800 pt-safe transition-[width] duration-200 lg:flex ${
      ...
      >
        <div className={`flex h-16 items-center border-b border-white/10 ${collapsed ? "justify-center px-2" : "justify-between gap-2 px-4"}`}>
          ...
          {collapsed && (
            <Link to="/admin" aria-label={appName} className="flex h-9 w-9 items-center justify-center rounded-[var(--radius-btn)] bg-accent-500 text-white">
              <AdminIcon name="logo" className="h-5 w-5" />
            </Link>
          )}
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-30 border-b-2 border-brand-800 bg-white/95 pt-safe backdrop-blur">
          <div className="flex h-16 items-center gap-3 px-4 lg:px-6">
            {/* Mobile menu toggle (desktop layout hides it visually, kept for ARIA/structural tests) */}
            <button
              type="button"
              className="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-lg text-ink hover:bg-slate-100 lg:hidden"
            ...
            <div className="min-w-0 flex-1 lg:hidden">
              <Brand appName={appName} locale={locale} onLight />
            </div>
            ...
            <div className="ms-auto flex shrink-0 items-center gap-1.5">
              ...
              <a
                href="/"
                className="hidden min-h-11 items-center gap-1.5 rounded-lg px-3 text-sm font-semibold text-ink-muted hover:bg-slate-100 hover:text-ink md:inline-flex"
              >
        ...
        <footer className="mt-8 border-t border-line bg-white px-6 py-5 text-center text-xs text-ink-muted">
      ...
      {mobileOpen && (
        <div className="lg:hidden">
          ...
          <nav
            id="admin-mobile-nav"
            aria-label={t(locale, "nav.menu")}
            className="fixed inset-y-0 start-0 z-50 flex w-72 max-w-[85vw] flex-col overflow-y-auto border-e border-brand-800 bg-brand-800 p-4 pt-safe"
          >
