    ...
    <div className="flex flex-col gap-6">
        ...
        <CardBody className="space-y-3">
            ...
            <div key={u.id} className="flex flex-wrap items-center justify-between gap-2 border-b border-line py-2.5 text-sm last:border-0" data-testid="teacher-row">
              <div className="flex min-w-0 items-center gap-3">
                <div className="flex min-w-0 flex-col">
                  <Link to={`/admin/users/${u.id}`} className="truncate font-medium text-ink hover:underline">
                    {u.fullName}
                  </Link>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between pt-2 text-sm">
              {teachers.page > 1 ? (
                <Link className="text-ink hover:underline" to={withPage(teachers.page - 1)}>{t(locale, "teachers.prevPage")}</Link>
              ) : <span />}
              ...
              {teachers.page < totalPages ? (
                <Link className="text-ink hover:underline" to={withPage(teachers.page + 1)}>{t(locale, "teachers.nextPage")}</Link>
              ) : <span />}
            </div>
          )}
        </CardBody>
      ...
      <Card>
        <CardBody className="space-y-3">
          <div className="flex items-center justify-between gap-3">
            ...
            {!canManage && <span className="text-xs text-ink-muted">{t(locale, "teachers.matrixReadOnly")}</span>}
          </div>
      ...
      >
        ...
        <div className="mt-4 flex justify-end gap-2">
          <button type="button" onClick={() => setSuspendId(null)} className="rounded-lg border border-line px-4 py-2 text-sm font-medium text-ink hover:bg-slate-50">
