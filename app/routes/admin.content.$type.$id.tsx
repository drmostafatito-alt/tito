      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {publicUrl && (
          <a
            ...
            className="inline-flex min-h-8 items-center gap-1 rounded-lg border border-line bg-surface px-2.5 text-xs font-medium text-ink-muted hover:bg-sand-100"
          >
          ...
          action={
            <div className="flex flex-wrap gap-2">
              <Form method="post">
                ...
                <button className="rounded-lg border border-line px-3 py-1.5 text-sm hover:bg-sand-100" disabled={nav.state === "submitting"}>↑ {t(locale, "content.moveUp")}</button>
              </Form>
              <Form method="post">
                <input type="hidden" name="_action" value="move-down" />
                <button className="rounded-lg border border-line px-3 py-1.5 text-sm hover:bg-sand-100" disabled={nav.state === "submitting"}>↓ {t(locale, "content.moveDown")}</button>
              </Form>
              {canDuplicate && (
                <Form
                  method="post"
                ...
                >
                  ...
                  <button className="rounded-lg border border-line px-3 py-1.5 text-sm hover:bg-sand-100" disabled={nav.state === "submitting"}>⧉ {t(locale, "content.duplicate")}</button>
                </Form>
              )}
              <Form method="post">
                <input type="hidden" name="_action" value="archive" />
                <button className="rounded-lg border border-error/30 px-3 py-1.5 text-sm text-error hover:bg-error-soft" disabled={nav.state === "submitting"}>{t(locale, "content.archive")}</button>
              </Form>
            </div>
          }
        />
          ...
          <Form method="post" className="grid gap-3 sm:grid-cols-2">
            ...
            <div className="sm:col-span-2 flex items-center gap-3">
              ...
              {actionData && "error" in actionData && actionData.error && (
                <span className="text-sm text-error">{String(actionData.error)}</span>
              )}
            </div>
          </Form>
        </CardBody>
            ...
            <Form method="post" className="space-y-3">
                  ...
                  {prereqCandidates.map((c) => {
                    ...
                    return (
                      <label key={c.courseId} className="flex items-center gap-2 text-sm">
                        ...
                        {c.slug && <span className="text-xs text-sand-400">/{c.slug}</span>}
                      </label>
                    );
                  })}
                </fieldset>
              ...
              <div className="flex items-center gap-3">
                ...
                {actionData && "error" in actionData && actionData.error === "prereq_cycle" && (
                  <span className="text-sm text-error">{t(locale, "content.prereqCycleError")}</span>
                )}
              </div>
            </Form>
          </CardBody>
          ...
          <CardBody>
            {!outline || outline.length === 0 ? (
              <div className="flex flex-col items-center gap-2 rounded-xl border border-dashed border-line bg-sand-100/60 px-6 py-8 text-center">
                <span className="text-3xl text-sand-300" aria-hidden="true">📚</span>
                <p className="font-medium text-ink-soft">{t(locale, "content.noUnits")}</p>
              ...
              <ol className="flex flex-col gap-3">
                {outline.map((u, ui) => (
                  <li key={u.id} className="overflow-hidden rounded-xl border border-line">
                    <div className="flex flex-wrap items-center gap-2 border-b border-line bg-sand-100 px-3 py-2">
                      ...
                      <Link to={`/admin/content/unit/${u.id}`} className="text-sm font-semibold text-ink hover:text-brand-700 hover:underline">
                      ...
                      <ul className="flex flex-col">
                        {u.lessons.map((l) => (
                          <li key={l.id} className="flex items-center gap-2 border-b border-line px-4 py-2 text-sm last:border-0">
                            <Link to={`/admin/content/lesson/${l.id}`} className="flex min-w-0 flex-1 items-center gap-2 text-ink-soft hover:text-brand-700 hover:underline">
            ...
            <Form method="post" className="grid gap-3 sm:grid-cols-4">
              ...
              <div className="sm:col-span-4 flex flex-wrap items-center gap-3">
                ...
                {actionData && "error" in actionData && actionData.error === "link_invalid" && (
                  <span className="text-sm text-error" data-testid="link-error">{t(locale, "content.linkInvalid")}</span>
                )}
              </div>
            </Form>
          </CardBody>
