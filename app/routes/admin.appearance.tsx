...
const selectCls = "h-[42px] rounded-lg border border-line bg-white px-3 text-sm focus:border-brand-500 focus:outline-none";
...
function SocialHub({ identity, locale, L }: { identity: { socialLinks?: SocialLink[]; facebook: string; youtube: string; instagram: string; tiktok: string; twitter: string; linkedin: string; telegram: string }; locale: Loc; L: (k: string) => string }) {
  ...
  return (
    <fieldset className="sm:col-span-2 flex flex-col gap-3 rounded-lg border border-line p-4">
      <legend className="px-1 text-sm font-semibold text-ink">{L("cms.ui.socialHub")}</legend>
      {rows.map((row, i) => (
        <div key={row.id} className="grid gap-2 rounded-lg border border-line bg-slate-50 p-3 sm:grid-cols-2">
          ...
          <div className="flex flex-wrap items-center gap-2 sm:col-span-2">
            <button type="button" className="inline-flex min-h-9 items-center rounded-lg border border-line bg-white px-2.5 text-xs" onClick={() => move(i, -1)} disabled={i === 0} aria-label="↑">↑</button>
            <button type="button" className="inline-flex min-h-9 items-center rounded-lg border border-line bg-white px-2.5 text-xs" onClick={() => move(i, 1)} disabled={i === rows.length - 1} aria-label="↓">↓</button>
...
function ColorInput({ name, value, label }: { name: string; value: string; label: string }) {
  return (
    <div className="flex flex-col">
      <span className="mb-1 text-sm font-medium text-ink">{label}</span>
      <div className="flex items-center gap-2">
        <input type="color" name={name} defaultValue={value} className="h-10 w-14 cursor-pointer rounded border border-line bg-white p-1" />
...
function IdentityEditor({
  ...
  const Sec = ({ k }: { k: string }) => (
    <h3 className="col-span-full mt-4 border-b border-line pb-1.5 text-sm font-semibold text-ink first:mt-0">
        ...
        {TABS.map((tb) => {
          ...
          return (
            <Link
              ...
              className={`inline-flex min-h-11 items-center rounded-lg px-4 text-sm font-medium ${tab === tb ? "bg-brand-700 text-white" : "border border-line bg-white text-ink hover:bg-slate-50"} ${allowed ? "" : "opacity-50"}`}
            >
              {L(key)}
            </Link>
          );
        ...
        <Card>
          ...
          <CardBody>
            <Form method="post" className="grid gap-4 sm:grid-cols-3">
              ...
              <div className="flex flex-col">
                <span className="mb-1 text-sm font-medium text-ink">{L("cms.set.fontScale")}</span>
              ...
              <div className="flex flex-col">
                <span className="mb-1 text-sm font-medium text-ink">{L("cms.set.headingFont")}</span>
              ...
              <div className="flex flex-col">
                <span className="mb-1 text-sm font-medium text-ink">{L("cms.set.bodyFont")}</span>
          ...
          <CardBody>
            <Form method="post" className="flex flex-col gap-6">
              ...
              <fieldset className="flex flex-col gap-2 rounded-lg border border-line p-4">
                <legend className="px-1 text-sm font-semibold text-ink">{L("cms.set.courseCard")}</legend>
                ...
                <div className="grid gap-3 sm:grid-cols-3">
                  ...
                  <div className="flex flex-col">
                    <span className="mb-1 text-sm font-medium text-ink">{L("cms.set.layout")}</span>
              ...
              <fieldset className="flex flex-col gap-2 rounded-lg border border-line p-4">
                <legend className="px-1 text-sm font-semibold text-ink">{L("cms.set.subjectCard")}</legend>
              ...
              <fieldset className="flex flex-col gap-2 rounded-lg border border-line p-4">
                <legend className="px-1 text-sm font-semibold text-ink">{L("cms.set.lessonPage")}</legend>
          ...
          <CardBody>
            <Form method="post" className="flex flex-col gap-3">
              ...
              <fieldset className="flex flex-col gap-1 rounded-lg border border-line p-4">
                <legend className="px-1 text-sm font-semibold text-ink">{L("cms.set.modules")}</legend>
                {dash.modules.map((m) => (
          ...
          <CardBody>
            <Form method="post" className="flex flex-col gap-4">
              ...
              <fieldset className="flex flex-col gap-3 rounded-lg border border-line p-4">
                <legend className="px-1 text-sm font-semibold text-ink">{L("cms.ui.systemPlatform")}</legend>
              ...
              <fieldset className="flex flex-col gap-3 rounded-lg border border-line p-4" data-testid="language-settings">
                <legend className="px-1 text-sm font-semibold text-ink">{L("cms.ui.systemLanguage")}</legend>
                <p className="text-xs text-ink-muted">{L("cms.f.localeHint")}</p>
                <div className="flex flex-col">
                  <span className="mb-1 text-sm font-medium text-ink">{L("cms.f.defaultLocale")}</span>
              ...
              {loaderData.isSuper && (
                <fieldset className="flex flex-col gap-3 rounded-lg border border-line p-4">
                  <legend className="px-1 text-sm font-semibold text-ink">{L("cms.ui.systemVideo")}</legend>
                  <div className="flex flex-col">
                    <span className="mb-1 text-sm font-medium text-ink">{L("cms.f.videoProvider")}</span>
              ...
              {loaderData.isSuper && (
                <fieldset className="flex flex-col gap-3 rounded-lg border border-line p-4" data-testid="payments-settings">
                  <legend className="px-1 text-sm font-semibold text-ink">{t(locale, "commerceAdmin.settingsTitle")}</legend>
                  <label className="flex items-center gap-2 text-sm">
                    <input type="checkbox" name="manualEnabled" defaultChecked={pay.manualEnabled} />
                    {t(locale, "commerceAdmin.manualEnabled")}
                  </label>
                  <div className="flex flex-col">
                    <span className="mb-1 text-sm font-medium text-ink">{t(locale, "commerceAdmin.manualInstructionsAr")}</span>
                  ...
                  <div className="flex flex-col">
                    <span className="mb-1 text-sm font-medium text-ink">{t(locale, "commerceAdmin.manualInstructionsEn")}</span>
