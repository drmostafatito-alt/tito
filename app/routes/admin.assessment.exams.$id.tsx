    ...
    return (
      <div className="space-y-4" key={`exam-${params.id ?? "new"}`}>
        ...
        <Form method="post" className="max-w-xl space-y-3">
          ...
          <div>
            <label htmlFor="exam-attach-kind-new" className="text-sm font-medium text-ink-soft">{t(locale, "assessment.lessonLink")}</label>
    ...
    <div className="space-y-4">
      ...
      <Form method="post">
        ...
        <Card>
          <CardBody className="space-y-3">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div>
                <label htmlFor="exam-attach-kind" className="text-sm font-medium text-ink-soft">{t(locale, "assessment.lessonLink")}</label>
      ...
      <Form method="post">
        ...
        <Card>
          <CardBody className="space-y-3">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div>
                <label htmlFor="exam-show" className="text-sm font-medium text-ink-soft">{t(locale, "assessment.showResults")}</label>
              ...
              <div>
                <label htmlFor="exam-mode" className="text-sm font-medium text-ink-soft">{t(locale, "assessment.selectionMode")}</label>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div>
                <label htmlFor="exam-starts-at" className="text-sm font-medium text-ink-soft">{t(locale, "assessment.startsAt")} (UTC)</label>
              ...
              <div>
                <label htmlFor="exam-ends-at" className="text-sm font-medium text-ink-soft">{t(locale, "assessment.endsAt")} (UTC)</label>
            ...
            <details open={cfg.mode === "pool"} className="rounded-lg border border-line">
              <summary className="cursor-pointer px-3 py-2 text-sm font-semibold text-ink-soft">{t(locale, "assessment.poolsTitle")}</summary>
        ...
        <Card data-testid="pool-preview">
          <CardBody className="space-y-3">
            ...
            <div className="grid gap-2 text-xs text-ink-muted sm:grid-cols-2">
              <div className="rounded-lg border border-line bg-sand-100/60 p-2">
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[520px] border-collapse text-xs">
                <thead>
                  <tr className="border-b text-start text-ink-muted">
                ...
                <tbody>
                  {poolPreview.pools.map((p, i) => (
                    <tr key={i} className="border-b border-line">
                      ...
                      <td className={`py-1 pe-2 ${p.eligible < p.count ? "font-bold text-error" : ""}`}>{p.eligible}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            </div>
            <p className="text-xs text-ink-muted">{t(locale, "assessment.poolPreviewNote")}</p>
          </CardBody>
        </Card>
      )}
      ...
      <Card>
        <CardBody className="space-y-3">
          ...
          {cfg.mode === "pool" && <p className="text-xs text-warning">{t(locale, "assessment.mode_pool")}</p>}
          {attached.length === 0 && <p className="text-sm text-ink-muted">{t(locale, "assessment.noQuestions")}</p>}
          <ol className="space-y-2">
            {attached.map((a, i) => (
              <li key={a.questionId} className="flex flex-wrap items-center gap-2 rounded-lg border p-2.5 text-sm">
                <span className="w-6 text-center text-xs text-ink-muted">{i + 1}</span>
                ...
                {draft && perms.edit ? (
                  <div className="flex items-center gap-1">
                    <Form method="post" className="flex items-center gap-1">
                      ...
                      <input name="points" type="number" step="0.5" min="0.5" max="1000" defaultValue={a.points} className="h-9 w-20 rounded-lg border border-line px-2 text-xs" aria-label={t(locale, "assessment.examPoints")} />
          ...
          {draft && perms.edit && cfg.mode === "manual" && (
            <Form method="post" className="flex flex-wrap items-end gap-2 border-t pt-3">
              ...
              <div className="min-w-0 flex-1">
                <label className="text-sm font-medium text-ink-soft">{t(locale, "assessment.pickQuestion")}</label>
      ...
      <Card>
        ...
        <CardBody>
          {attempts.length === 0 ? (
            <div className="flex flex-col items-center gap-2 rounded-xl border border-dashed border-line bg-sand-100/60 px-6 py-8 text-center">
              <span className="text-3xl text-sand-300" aria-hidden="true">🗒️</span>
              <p className="font-medium text-ink-muted">{t(locale, "assessment.noAttempts")}</p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full min-w-[640px] border-collapse text-sm">
                <thead>
                  <tr className="border-b border-line text-left text-xs text-ink-muted rtl:text-right">
                ...
                <tbody>
                  {attempts.map((a) => (
                    <tr key={a.id} className="border-b border-line align-middle last:border-0 hover:bg-sand-100/60">
                      <td className="px-3 py-2.5">
                        <p className="font-medium text-ink">{a.studentName}</p>
                        <p className="text-xs text-ink-muted" dir="ltr">{a.studentEmail}</p>
                      </td>
                      <td className="px-3 py-2.5 text-ink-muted">#{a.attemptNumber}</td>
                      <td className="px-3 py-2.5">
                      ...
                      <td className="px-3 py-2.5 text-ink-soft">
                        ...
                        {a.percentage !== null && <span className="ml-1 text-xs text-ink-muted rtl:mr-1">({a.percentage}%)</span>}
                      </td>
                      <td className="px-3 py-2.5">
                        {a.passed === null ? (
                          <span className="text-ink-muted">—</span>
                        ) : a.passed ? (
                          <span className="inline-flex items-center gap-1 font-medium text-success">{t(locale, "assessment.resultPass")}</span>
                        ) : (
                          <span className="font-medium text-error">{t(locale, "assessment.resultFail")}</span>
                        )}
                      </td>
                      <td className="px-3 py-2.5 text-end">
                        <Link to={`/admin/assessment/attempts/${a.id}`} className="inline-flex min-h-9 items-center rounded-lg border border-line px-3 text-xs font-medium text-ink-soft hover:border-brand-400 hover:text-brand-700">
