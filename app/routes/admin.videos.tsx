        ...
        <CardBody className="space-y-4">
          ...
          <Form method="post" className="grid gap-3 border-t border-line pt-4" data-testid="youtube-form">
            ...
            <div className="sm:col-span-3">
              <h2 className="text-sm font-semibold text-ink">{t(locale, "videosAdmin.youtubeHeading")}</h2>
            ...
            <label className="grid gap-1 text-sm sm:col-span-3">
              ...
              />
              <span className="text-xs text-ink-muted">{t(locale, "videosAdmin.youtubeUrlHint")}</span>
            </label>
            <label className="grid gap-1 text-sm">
          ...
          <ul className="space-y-2">
            {loaderData.videos.map((v) => (
              <li key={v.id} className="flex flex-wrap items-center gap-2 text-sm">
                ...
                {v.youtubeId && <span className="font-mono text-xs text-ink-muted" data-testid="youtube-id">{v.youtubeId}</span>}
                {v.status !== "ready" && (
                  <Form method="post" className="inline">
                    <input type="hidden" name="_action" value="sync" />
                    <input type="hidden" name="videoId" value={v.id} />
                    <button className="rounded border border-line px-2 py-1 text-xs hover:bg-slate-50">
