  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b-2 border-brand-800 pb-4">
        <h1 className="sig-display text-3xl text-ink">{t(locale, "notifications.title")}</h1>
        {loaderData.unread > 0 && (
          <Form method="post">
            ...
            <Card key={a.id}>
              <CardBody className="flex flex-col gap-2">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <h2 className="flex items-center gap-2 text-base font-bold text-ink" data-testid={`notif-title-${a.id}`}>
                    {!a.readAt && <span aria-hidden="true" className="inline-block h-2.5 w-2.5 shrink-0 bg-accent-500" />}
                    {title}
                  </h2>
                  <span className="flex items-center gap-2">
                    ...
                    <span className="text-xs tabular-nums text-ink-muted">{formatDate(locale, a.publishedAt ?? a.createdAt)}</span>
                  </span>
                </div>
                {body && <p className="whitespace-pre-line text-sm leading-relaxed text-ink-muted" data-testid={`notif-body-${a.id}`}>{body}</p>}
                {a.expiresAt && (
                  <p className="text-xs tabular-nums text-ink-muted">{t(locale, "notifications.expiresAt")}: {formatDate(locale, a.expiresAt)}</p>
                )}
                {!a.readAt && (
                  <Form method="post">
