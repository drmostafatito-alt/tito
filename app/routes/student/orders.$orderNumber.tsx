  ...
  return (
    <div className="space-y-4">
      <nav className="flex flex-wrap items-center gap-x-1.5 gap-y-1 text-sm text-ink-muted" aria-label={t(locale, "common.breadcrumb")}>
        <Link to="/orders" className="font-semibold text-ink"><span className="sig-u">{t(locale, "commerce.myOrders")}</span></Link>
        <span aria-hidden="true">›</span>
        <span dir="ltr" className="tabular-nums">{order.orderNumber}</span>
      </nav>

      <div className="flex flex-wrap items-center justify-between gap-2 border-b-2 border-brand-800 pb-4">
        <h1 className="sig-display text-3xl text-ink">{t(locale, "commerce.orderTitle")}</h1>
        <Badge
          tone={
      ...
      <Card>
        ...
        <CardBody className="space-y-2 text-sm">
          {items.map((i, idx) => (
            <div key={idx} className="flex items-center justify-between gap-2 font-medium text-ink">
              <span>{locale === "ar" ? i.titleAr : i.titleEn}</span>
              <span dir="ltr" className="tabular-nums">{formatMoney(i.unitPriceMinor, order.currency)}</span>
            </div>
          ))}
          <div className="border-t border-line pt-2">
            ...
            <div className="flex items-center justify-between font-bold tabular-nums text-ink" data-testid="order-total-row">
        ...
        <Card>
          ...
          <CardBody className="space-y-3 text-sm">
            ...
            {(locale === "ar" ? latest.instructions.instructionsAr : latest.instructions.instructionsEn) ? (
              <p className="whitespace-pre-line rounded-[var(--radius-btn)] border-s-2 border-s-brand-800 bg-slate-50 p-3 leading-relaxed text-ink" data-testid="instructions-text">
                {locale === "ar" ? latest.instructions.instructionsAr : latest.instructions.instructionsEn}
              </p>
            ) : (
            ...
            {latest.evidence && (latest.status === "under_review" || latest.status === "paid" || latest.status === "failed") && (
              <dl className="space-y-1 rounded-[var(--radius-btn)] border border-line bg-slate-50 p-3 text-sm" data-testid="submitted-evidence">
                ...
                {latest.proofPreview ? (
                  <div className="flex flex-col gap-2 pt-1">
                    <dt className="font-semibold text-ink-muted">{t(locale, "commerce.proofImage")}</dt>
                    <img src={latest.proofPreview.url} alt={t(locale, "commerce.proofImageAlt")} className="max-h-48 w-full rounded-[var(--radius-btn)] border border-line bg-white object-contain" data-testid="proof-preview-img" />
                  </div>
                ) : null}
              </dl>
            )}
            ...
            {canConfirm && (
              <Form method="post" encType="multipart/form-data" className="space-y-3 pt-1">
                ...
                <label className="grid gap-1.5 text-sm font-semibold text-ink">
                  <span>{t(locale, "commerce.senderName")} <span className="text-xs font-normal text-ink-muted">({t(locale, "commerce.optional")})</span></span>
                  <input name="senderName" dir="auto" maxLength={200} className="rounded-[var(--radius-btn)] border border-line bg-white px-3 py-2.5 focus:border-brand-800 focus:outline-none" />
                </label>
                <div className="grid gap-3 sm:grid-cols-2">
                  <label className="grid gap-1 text-sm">
                    <span>{t(locale, "commerce.transferAmount")} <span className="text-xs font-normal text-ink-muted">({t(locale, "commerce.mustEqualTotal")})</span></span>
                    <input name="transferAmount" type="number" inputMode="decimal" step="0.01" min="0" dir="ltr" className="rounded-[var(--radius-btn)] border border-line bg-white px-3 py-2.5 focus:border-brand-800 focus:outline-none" placeholder={formatMoney(order.totalMinor, order.currency).replace(/[^\d.,\s]/g, "").trim()} />
                  </label>
                  <label className="grid gap-1 text-sm">
                    <span>{t(locale, "commerce.transferDate")} <span className="text-xs font-normal text-ink-muted">({t(locale, "commerce.optional")})</span></span>
                    <input type="datetime-local" name="transferDate" className="rounded-[var(--radius-btn)] border border-line bg-white px-3 py-2.5 focus:border-brand-800 focus:outline-none" />
                  </label>
                </div>
                <label className="grid gap-1 text-sm">
                  <span>{t(locale, "commerce.proofFile")} <span className="text-xs font-normal text-ink-muted">({t(locale, "commerce.optional")})</span></span>
                  <input type="file" name="proofFile" accept="image/png,image/jpeg,image/webp" className="block w-full text-sm text-ink-muted file:me-3 file:rounded-[var(--radius-btn)] file:border-0 file:bg-brand-800 file:px-4 file:py-2 file:text-sm file:font-semibold file:text-white" />
                </label>
                <label className="grid gap-1.5 text-sm font-semibold text-ink">
                  <span>{t(locale, "commerce.noteOptional")}</span>
                  <input name="note" dir="auto" maxLength={500} className="rounded-[var(--radius-btn)] border border-line bg-white px-3 py-2.5 focus:border-brand-800 focus:outline-none" />
                </label>
                <p className="text-xs leading-relaxed text-ink-muted">{t(locale, "commerce.proofPrivacy")}</p>
                ...
                <p className="text-xs leading-relaxed text-ink-muted">{t(locale, "commerce.confirmDisclaimer")}</p>
              </Form>
            )}
          </CardBody>
        </Card>
      ...
      <Card>
        ...
        <CardBody className="space-y-2 text-sm">
          {payments.map((p) => (
            <div key={p.id} className="flex flex-wrap items-center justify-between gap-2 border-b border-line py-2 last:border-b-0" data-testid="payment-row">
              <span className="text-xs tabular-nums text-ink-muted" dir="ltr">
                {p.provider} · {formatDate(locale, p.createdAt)}
              </span>
              <span className="flex items-center gap-2">
                <span dir="ltr" className="text-xs font-semibold tabular-nums text-ink">{formatMoney(p.amountMinor, p.currency)}</span>
