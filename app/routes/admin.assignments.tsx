...
const inputCls = "h-[42px] rounded-lg border border-line bg-surface px-3 text-sm";
      ...
      <div className="flex gap-2 border-b">
        {(["assignments", "grading"] as const).map((tb) => (
          <Link
            ...
            className={`border-b-2 px-4 py-2 text-sm font-medium ${tab === tb ? "border-brand-700 text-brand-700" : "border-transparent text-ink-muted hover:text-ink"}`}
          >
            ...
            {grading.length === 0 ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full min-w-[720px] border-collapse text-sm">
                  ...
                  <tbody>
                    {grading.map((s) => (
                      <tr key={s.submissionId} className="border-b border-line last:border-0 hover:bg-sand-100/60">
                        ...
                        <td className="px-3 py-2.5 text-end">
                          <Link to={`/admin/assignments/${s.assignmentId}`} className="inline-flex min-h-9 items-center rounded-lg border border-line px-3 text-xs font-medium text-brand-700 hover:border-brand-400">
              ...
              <CardBody className="p-0">
                <div className="overflow-x-auto">
                  <table className="w-full min-w-[820px] border-collapse text-sm">
                    ...
                    <tbody>
                      {list.items.map((a) => (
                        <tr key={a.id} className="border-b border-line last:border-0 hover:bg-sand-100/60">
          ...
          {list.total > list.limit && (
            <div className="flex items-center justify-between gap-2 text-sm">
              ...
              <div className="flex gap-2">
                {list.offset > 0 && (
                  <Link to={`/admin/assignments${buildQuery({ status, q, courseId, offset: String(Math.max(list.offset - list.limit, 0)) })}`} className="rounded-lg border border-line px-3 py-1.5 text-sm font-medium text-ink-soft hover:bg-sand-100">
                    {t(locale, "assignment.prev")}
                  </Link>
                )}
                {list.offset + list.limit < list.total && (
                  <Link to={`/admin/assignments${buildQuery({ status, q, courseId, offset: String(list.offset + list.limit) })}`} className="rounded-lg border border-line px-3 py-1.5 text-sm font-medium text-ink-soft hover:bg-sand-100">
