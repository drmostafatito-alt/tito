...
const inputCls = "h-[42px] rounded-lg border border-line bg-surface px-3 text-sm focus:border-brand-500 focus:outline-none";
            ...
            <CardBody>
              <Form method="post" className="grid gap-3 sm:grid-cols-[1fr_1fr_1fr_auto] sm:items-end">
                ...
                <div className="flex flex-col">
                  <span className="mb-1 text-sm font-medium text-ink-soft">{L("cms.ui.actionType")}</span>
            ...
            <Card>
              ...
              <CardBody>
                <Form method="post" className="grid gap-3 sm:grid-cols-2">
                  ...
                  <div className="flex flex-col">
                    <span className="mb-1 text-sm font-medium text-ink-soft">{L("cms.ui.formStatus")}</span>
              ...
              <CardBody>
                {fields.length === 0 ? (
                  ...
                  <ul className="flex flex-col gap-2">
                    {fields.map((fd) => (
                      <li key={String(fd.id)} className={`rounded-lg border border-line px-3 py-2 ${fd.enabled ? "bg-surface" : "bg-sand-100 opacity-60"}`}>
                        ...
                        <details className="mt-1">
                          ...
                          <Form method="post" className="mt-2 grid gap-3 sm:grid-cols-2">
                            ...
                            <div className="flex flex-col">
                              <span className="mb-1 text-sm font-medium text-ink-soft">{L("cms.ui.optionsJson")}</span>
                              <textarea name="optionsJson" rows={3} dir="ltr" defaultValue={fd.options ? JSON.stringify(fd.options) : ""} placeholder={L("cms.ui.optionsHint")} className="rounded-lg border border-line px-3 py-2 font-mono text-xs focus:border-brand-500 focus:outline-none" />
                            </div>
                            <div className="flex flex-col">
                              <span className="mb-1 text-sm font-medium text-ink-soft">{L("cms.ui.validationJson")}</span>
                              <textarea name="validationJson" rows={3} dir="ltr" defaultValue={fd.validation ? JSON.stringify(fd.validation) : ""} placeholder={L("cms.ui.validationHint")} className="rounded-lg border border-line px-3 py-2 font-mono text-xs focus:border-brand-500 focus:outline-none" />
                            </div>
                            <label className="inline-flex min-h-11 items-center gap-2 text-sm font-medium text-ink-soft">
            ...
            <Card>
              ...
              <CardBody>
                <Form method="post" className="grid gap-3 sm:grid-cols-2">
                  ...
                  <div className="flex flex-col">
                    <span className="mb-1 text-sm font-medium text-ink-soft">{L("cms.ui.fieldType")}</span>
                  ...
                  <ul className="flex flex-col gap-2">
                    {submissions.map((sub) => (
                      <li key={String(sub.id)} className="rounded-lg border border-line px-3 py-2 text-xs">
                        ...
                        <pre className="mt-1 overflow-x-auto whitespace-pre-wrap text-ink-soft" dir="auto">{JSON.stringify(sub.data, null, 1)}</pre>
                      </li>
                    ))}
                  </ul>
                )}
