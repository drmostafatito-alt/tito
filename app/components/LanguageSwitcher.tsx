...
export function LanguageSwitcher({ locale, options }: { locale: Locale; options?: Locale[] }) {
  ...
  return (
    <Form method="post" action="/set-locale" reloadDocument replace data-locale-switch="">
      ...
      <button
        type="submit"
        className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-lg px-3 text-sm font-bold text-ink-soft transition-colors hover:bg-sand-100 hover:text-ink"
