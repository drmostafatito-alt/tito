    ...
    <div className="flex flex-wrap gap-1.5" data-testid="range-switcher">
      {ranges.map((r) => (
        <Link
          ...
          to={`${base}?range=${r}`}
          data-testid={`range-${r}`}
          className={`inline-flex min-h-11 items-center rounded-lg px-3 py-2 text-sm font-medium ${r === range ? "bg-brand-600 text-white" : "bg-white text-slate-600 ring-1 ring-slate-200 hover:bg-slate-50"}`}
        >
