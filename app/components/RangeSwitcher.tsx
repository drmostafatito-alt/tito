    ...
    <div className="flex flex-wrap gap-1.5" data-testid="range-switcher">
      {ranges.map((r) => (
        <Link
          ...
          to={`${base}?range=${r}`}
          data-testid={`range-${r}`}
          className={`inline-flex min-h-9 items-center rounded-lg px-3 py-1.5 text-sm font-medium ${r === range ? "bg-brand-800 font-bold text-white" : "bg-white font-semibold text-ink-muted ring-1 ring-line hover:bg-slate-50 hover:text-ink"}`}
        >
